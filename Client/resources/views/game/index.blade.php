...
<script>
    ...
    function renderViewV2() {
        ...
        const marketRateDataTemp = $('#marketRateDataTemp'); // Cache the target container

        Object.entries(matchListD.data.list.market).map(([k, v]) => {
            createMarketContainer(k, v);

            Object.entries(v.rate).map(([k1, v2]) => {
                createMarketRateContainer(k, v, k1, v2, marketRateDataTemp);
            });
        });
    }
    ...
    function createMarketContainer(k, v) {
        ...
        if (existingElement.length) {
            ...
            existingElement.find('.market_name').html('<i class="fa-sharp fa-solid fa-star" style="color: #415a5b; margin-right: 0.5rem;"></i>' + v.market_name);
        } else {
            // Clone the template if the element doesn't exist
    ...
    function createMarketRateContainer(k, v, k1, v2, container) {
        // Find the existing element within the container using the market_bet_id
        const marketBetRateTemp = container.find(`[market_bet_id="${v2.market_bet_id}"]`);

        if (marketBetRateTemp.length) {
            // Update the text of existing elements
            switch (v.priority) {
            ...
            marketBetRateTemp.attr('market_type', v.market_name);
            marketBetRateTemp.attr('market_name', v2.market_bet_name);
