...
<script>
    ...
    function viewIni() { // view ini 
        ...
        Object.entries(fixtureData.list).sort(([, marketA], [, marketB]) => marketA.cateOrder - marketB.cateOrder).map(([k, v]) => {
            if( v?.list?.[v.base_main_line]?.length > 0 )  createMarketContainer(k, v);
            if (v.list) {
    ...
    function renderView() {
        ...
        Object.entries(fixtureData.list).sort(([, marketA], [, marketB]) => marketA.cateOrder - marketB.cateOrder).map(([k, v]) => {
            ...
            if( bet_div.length === 0 && v?.list?.[v.base_main_line]?.length > 0 ) createMarketContainer(k, v);
            if (v.list) {
                const sortedKeys = Object.keys(v.list)
                // 遍历排序后的数组
    ...
    function createScoreBoardTemplate(sport, data, baseballShowStage) {
        ...
        let stageStr = '';
        if (sport === 131506 && data?.periods?.period < 10) {
        ...
        for (let i = 0; i < gameTitle.length; i++) {
            ...
            const thHome = $(`<td class="${sport === 131506 ? 'isBsbll isBsbll_' + i : ''}" style="width:10%;text-align:center;">`).text(scoreValue !== undefined ? scoreValue : '-');
            if (!(sport === 131506 && baseballShowStage.indexOf(i) === -1)) {
        ...
        for (let i = 0; i < gameTitle.length; i++) {
            ...
            const thAway = $(`<td class="${sport === 131506 ? 'isBsbll isBsbll_' + i : ''}" style="width:10%;text-align:center;">`).text(scoreValue !== undefined ? scoreValue : '-');
            if (!(sport === 131506 && baseballShowStage.indexOf(i) === -1)) {
